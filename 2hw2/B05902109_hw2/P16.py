...
for iteration in range(250):
    ...
    for lamda_index in range(5):
        ...
        predict = np.sign(test_X_m.dot(w))
        for i in range(test_n):
            test_Y_predict[lamda_index][i] += predict[i]
for i in range(5):
    for j in range(test_n):
        if  np.sign(test_Y_predict[i][j]) != test_Y[j] :
            E_out[i] += 1.0
            print(test_Y_predict[i][j])
    E_out[i] /= test_n
    print('lamda = %g\t, E_out = %g'%(lamda_array[i], E_out[i]))
