...
if __name__ == '__main__':    
    ...
    if arg.train:
        ...
        if arg.is_canceled:
            ...
            # gsearch1 = GridSearchCV(estimator = xgb_classifier, param_grid = param_test, scoring='f1_micro',n_jobs=1,verbose = 10, cv=5)
            ...
            xgb_classifier.fit(trainDataset.X, trainDataset.Y[:,0])
            print('[LOG] Fitting done!')
            print('-- Model Report --')
            print('XGBoost train Accuracy: '+str(accuracy_score(xgb_classifier.predict(trainDataset.X), trainDataset.Y[:,0])))
            print('XGBoost valid Accuracy: '+str(accuracy_score(xgb_classifier.predict(validDataset.X), validDataset.Y[:,0])))

            if not os.path.isdir(arg.img):
                os.mkdir(arg.img)
            f, ax = plt.subplots(figsize=(10,5))
            plot = sns.barplot(x=features, y=xgb_classifier.feature_importances_)
            ax.set_title('Feature Importance')
            plot.set_xticklabels(plot.get_xticklabels(),rotation='vertical')
            plt.savefig(os.path.join(arg.img, modelName))

            # save model
            pickl = {'model': xgb_classifier}
        ...
        if arg.adr:
            ...
            # }
            # #metrics to consider: f1_micro, f1_macro, roc_auc_ovr
            # gsearch1 = GridSearchCV(estimator = xgb_regressor, param_grid = param_test, scoring='neg_mean_squared_error',n_jobs=1,verbose = 10, cv=5)
            ...
            print('[LOG] Fitting model...')
            xgb_regressor.fit(trainDataset.X, trainDataset.Y[:,1])
            print('[LOG] Fitting done!')
            print('-- Model Report --')
            print('XGBoost train MSE: '+str(mean_squared_error(xgb_regressor.predict(trainDataset.X), trainDataset.Y[:,1])))
            print('XGBoost valid MSE: '+str(mean_squared_error(xgb_regressor.predict(validDataset.X), validDataset.Y[:,1])))

            if not os.path.isdir(arg.img):
                os.mkdir(arg.img)
            f, ax = plt.subplots(figsize=(10,5))
            plot = sns.barplot(x=features, y=xgb_regressor.feature_importances_)
            ax.set_title('Feature Importance')
            plot.set_xticklabels(plot.get_xticklabels(),rotation='vertical')
            plt.savefig(os.path.join(arg.img, modelName))

            # save model
            pickl = {'model': xgb_regressor}
            if not os.path.isdir(arg.ckpt):
                os.mkdir(arg.ckpt)
            with open(os.path.join(arg.ckpt, modelName + '.pth'), 'wb') as fp:
                pickle.dump(pickl, fp)
